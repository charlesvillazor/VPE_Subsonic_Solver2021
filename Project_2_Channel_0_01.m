...
%April 28th, 2021
%MAE 456
%Final Project Part 2 - Channel Mach 0.01
clc;clear;close all
...
file = fopen('grid.bumps.txt'); %opening file
fgetl(file); %read and skip first line
data = cell2mat(textscan(file, '%f %f'));
Ny = 51; %number of terms in y direction
Nx = 201; %number of terms in x direction
...
for i = 1:Ny
    for j = 1:Nx
        x(j,i) = data(k,1);
        y(j,i) = data(k,2);
        k = k+1; %add one to every iteration
    end
end
...
%% Channel Vol Array Routine / Cell Centers Routine 
for i = 2:Nx %For area of top half
    for j = 2:Ny %For area of the bottom half
...
%% Interpolating Channel Cell Centers
for i = 2:Nx
...
%% Plotting Channel Volume Contour Plot
figure(1)
contourf(xcenter(2:Nx+1,2:Ny+1),ycenter(2:Nx+1,2:Ny+1),vol(1:Nx,1:Ny),40,'edgecolor','none')
colorbar 
axis equal
title('Channel Contour Plot of Volume')
xlabel('X Distance')
ylabel('Y Distance')

%% Channel Velocity Potential (Mach 0.01)
...
%% Boundary Contitions for Channel Phi (Mach 0.01)
%lecture 21 slide 6
%Channel Inflow
i = 1;
for j = 2:Ny
...
% %% Calculating Channel Velocity Gradient (Velocity Components)
% for j = 2:Ny
%     for i = 2:Nx
...
%         sum(1:2) = ((phi1*del(i,j,1,1:2))-(phi3*del(i-1,j,1,1:2))+(phi2*del(i,j,2,1:2))-(phi4*del(i,j-1,2,1:2)));
%         grad(i,j,:) = sum(:)./(vol(i,j));
%     end
% end

%% Alternate Channel Grad (Mach 0.01)
%Taken from website
for j = 2:Ny
    for i = 2:Nx
...
%% Boundary Conditions for Channel Grad (Mach 0.01)
%lecture 21 slide 6
%Channel Inflow
i = 1;
for j = 2:Ny
...
for i = 2:Nx
    ...
    grad(i,j,1) = grad(i,j+1,1)-(2*xnx*vdotn);
    grad(i,j,2) = grad(i,j+1,2)-(2*xny*vdotn);
end

j = Ny+1;
for i = 2:Nx
    area = sqrt((del(i,j-1,2,1)^2)+(del(i,j-1,2,2)^2));
    xnx = del(i,j-1,2,1)/area;
    xny = del(i,j-1,2,2)/area;
    vdotn = (grad(i,j-1,1)*xnx)+(grad(i,j-1,2)*xny);
    grad(i,j,1) = grad(i,j-1,1)-(2*xnx*vdotn);
    grad(i,j,2) = grad(i,j-1,2)-(2*xny*vdotn);
end

%% Contour plot of u component of Channel velocity (Mach 0.01)
figure(2)
...
colorbar;
axis equal
title('Channel Contour Plot of u component of Velocity (Mach 0.01)')
xlabel('X Distance')
ylabel('Y Distance')

%% Contour plot of v component of Channel velocity (Mach 0.01)
figure(3)
contourf(xcenter(2:Nx,2:Ny),ycenter(2:Nx,2:Ny),-grad(2:Nx,2:Ny,2),100,'edgecolor','none');
colorbar;
axis equal
title('Channel Contour Plot of v component of Velocity (Mach 0.01)')
xlabel('X Distance')
ylabel('Y Distance')

%% Calculating Channel Rho (Mach 0.01)
for i = 1:Nx+1
    for j = 1:Ny+1
...
kmax = 5000; %Maximum amount of iterations
tolerance = 1e-3; %tolerance for resnorm
mrelax = 7; %number of relaxation steps
omega = 1.5; %0.7:1512, 1:1512, 
...
colorbar;
axis equal
title('Channel Contour Plot of u component of Velocity (Mach 0.01)(After Iterating)')
xlabel('X Distance')
ylabel('Y Distance')
...
colorbar;
axis equal
title('Channel Contour Plot of v component of Velocity (Mach 0.01)(After Iterating)')
xlabel('X Distance')
ylabel('Y Distance')
...
%% Coefficient of Pressure vs x distance plot (Top Wall)
figure(7)
plot((xcenter(2:Nx,2)/max(xcenter(2:Nx,2))),cp(2:Nx,2));
title('Coefficient of Pressure vs x/c Plot - Top Wall (Mach 0.01)')
ylabel('C_p')
xlabel('x/c')
...
title('Coefficient of Pressure vs x/c Plot - Bottom Wall (Mach 0.01)')
ylabel('C_p')
xlabel('x/c')
...
%% Mach Number along the top slip wall (j = 2)
figure(9)
plot((xcenter(2:Nx,2)/max(xcenter(2:Nx,2))),mach(2:Nx,2))
title('Mach Number Distribution along top Slip Wall (Mach 0.01)')
xlabel('x/c')
ylabel('Mach Number')
...
title('Mach Number Distribution along bottom Slip Wall (Mach 0.01)')
xlabel('x/c')
ylabel('Mach Number')
...
colorbar;
axis equal
title('Channel Contour Plot of Magnitude of Velocity (Mach 0.01)(After Iterating)')
xlabel('X Distance')
ylabel('Y Distance')